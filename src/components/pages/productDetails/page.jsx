            ...
            <div className=" flex flex-col lg:flex-row min-h-screen ">
                <div className="flex lg:w-1/2 ">
                    {produit?.images?.length > 0 ? (
                        <Image src={produit.images[currentImageIndex].src} alt={produit?.name} title={produit?.name} width={1920} height={1080} className="h-full w-fit object-cover " />
                    ) : (
                ...
                <div className="lg:w-full  p-6 bg-primary flex flex-col justify-center items-center lg:items-start gap-4">
                    <h1 className=" font-primary text-white text-2xl lg:text-4xl font-semibold uppercase">{produit.name}</h1>
                    ...
                    <div className=" flex flex-col-reverse lg:flex-row gap-4">
                        <button className=" bg-white hover:bg-primary hover:text-white text-sm  lg:text-lg duration-700 border-2 cursor-pointer border-white font-primary py-2 px-4 rounded-full   text-primary capitalize flex flex-row items-center justify-center gap-2"><ShoppingCart size={20} /> acheter maintenant avec {produit?.price ? new Intl.NumberFormat('de-DE').format(produit.price) * quantite + " dh" : "--"}</button>
                        <button onClick={handleSaveDetails} className=" bg-secondary hover:bg-primary text-sm lg:text-lg  hover:border-white hover:text-white  duration-700 border-2 cursor-pointer border-secondary font-primary py-2 px-4 rounded-full   text-white capitalize flex flex-row items-center justify-center gap-2"><ShoppingCart size={20} /> ajouter a panier  {produit?.price ? new Intl.NumberFormat('de-DE').format(produit.price) * quantite + " dh" : "--"}</button>
                    </div>
